...
while read -r _ f; do
    [ -n "$f" ] || continue
    test -f "$GOOD/bin/$f" || { echo "FAIL: $f not installed"; exit 1; }
done < "$REPO/SHA256SUMS"
for exe in brew-safe-upgrade brew-safe-install brew-safe-update; do
    test -x "$GOOD/bin/$exe" || { echo "FAIL: $exe not executable"; exit 1; }
done
...
test "$(cat "$BAD/bin/brew-safe-upgrade")" = "SENTINEL" || {
    echo "FAIL: pre-existing install was mutated despite verification failure"; exit 1
}
