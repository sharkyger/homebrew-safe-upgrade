...
def _stub(tmp_path, table: dict):
    ...
    stub.write_text(
        ...
        "[{'id': i, 'severity': 'MEDIUM', 'score': 6.5, 'source': 'NIST NVD'} for i in ids]}, sys.stdout)\n"
        "    sys.exit(1)\n"
...
def _scenario(mock_env):
    write_outdated(
        mock_env, [{"name": "gh", "installed_versions": ["2.98.0"], "current_version": "2.99.0"}]
    )
    write_formula_info(mock_env, "gh", "2.99.0")
    write_deps(mock_env, "gh", [])


def _run(mock_env, stub, input_text="n\n"):
    return run_upgrade(
        ["--no-deps"], env_extra={"DEPENDENCY_SECURITY_CHECK": str(stub)}, input_text=input_text
    )


def test_same_finding_set_is_its_own_state_and_upgrades(mock_env, tmp_path):
    _scenario(mock_env)
    stub = _stub(
        tmp_path,
        {("gh", "2.98.0"): ["CVE-2024-53858"], ("gh", "2.99.0"): ["CVE-2024-53858"]},
    )
    result = _run(mock_env, stub, input_text="y\n")
    out = result.stdout
    ...
    assert json.loads((mock_env / "outdated.json").read_text())["formulae"] == []


def test_order_of_findings_does_not_matter(mock_env, tmp_path):
    _scenario(mock_env)
    stub = _stub(
        tmp_path,
        {
    ...
    out = _run(mock_env, stub).stdout
    assert "[SAME] gh 2.99.0 -- same 2 finding(s)" in out, out


def test_candidate_only_finding_stays_blocked(mock_env, tmp_path):
...
def test_installed_clean_stays_blocked(mock_env, tmp_path):
    _scenario(mock_env)
...
def test_installed_scan_failure_stays_blocked(mock_env, tmp_path):
