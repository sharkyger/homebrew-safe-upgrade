...
- **A finding with no version scope now says so wherever it is listed.** A freshness hold prints the installed version's findings, and it printed them all identically — so `hugo` held with a bare `[CRITICAL] CVE-2026-51785 (CVSS 9.8)` directly under the sentence "0.166.0 does not demonstrably fix them". Those two statements read as a contradiction: a critical vulnerability that an upgrade inexplicably declines to fix. They were never in conflict. NVD has not tied that record to any version, which is *precisely why* no upgrade can be shown to fix it — but nothing in the output said so, and `nss` made it starker by listing a 2015 and a 2007 CVE against a 2026 release with no explanation.
  ...
  The records were in the right place: `partition_unactionable()` only demotes an unscoped finding to `unactionable` when the scanned version is the latest, and the age check scans the *installed* version, which by definition is not — so they correctly stay blocking, with `scoped: False` still on them. The defect was purely in the rendering. Unscoped findings are now marked `(no version scope)` on their own line, followed by a note counting them across the whole set rather than the five displayed (`5 of 6 finding(s) carry no version scope`), which is what turns the hold's reason from an apparent contradiction into an explanation. Only an explicit `scoped: false` is marked — an absent key means the source has no concept of CPE scoping (OSV, GitHub Advisory), and marking those would mark nearly everything. The counting line is prefixed `why:` rather than `note:`, because `note:` relays the scanner's own words and the scanner emits its own unscoped-findings note when it cannot resolve a latest version: two `note:` lines disagreeing about one set is worse than no line at all. `brew safe-install` carries its own copy of the renderer and gets the same fix, with a test pinning the two together — they have drifted before. The end-of-run blocked-dependency reason carries the marker too, since a dependency blocked solely by an unscoped finding taints every dependent. No hold or bypass decision changes: `finding_ids()` still folds both groups into the comparison, because an unscoped finding is not fixed by an upgrade and applies to both versions equally.

### Added

- **The everyday three-command routine is documented where people will actually find it.** `brew update` → `brew safe-update` → `brew safe-upgrade` now opens the **How it works** section, with the reasoning for each step and for the order. It was already in the README, but buried in **Updating** several hundred lines down, which is a poor place for the habit the tool is meant to be used through — the step most worth doing (updating the gate itself before asking it to judge anything) is also the easiest to skip. The **Updating** section now points at the canonical description instead of repeating it.

  One claim was corrected while moving it: the leading `brew update` was described as "optional — `safe-update` and `safe-upgrade` each run it internally". That is not what either does. `brew safe-update` runs no `brew update` of its own at all; only its *formula* route inherits one, indirectly, by handing off to `brew safe-upgrade --self`, and its script route never consults Homebrew metadata — it resolves the latest release from GitHub. `brew safe-upgrade` does run one, but deliberately *tolerates* failure so a transient tap error cannot abort an upgrade run: the error is still printed, but the exit status is discarded, so it scrolls past mid-run and every verdict afterwards is computed against whatever metadata happens to be on disk. `--self` is stricter and aborts. Running the refresh as its own step is what turns that from something you scroll past into something you act on.
...
- **The installed-version baseline now reads the keg you are actually running.** `brew outdated --json=v2` reports `installed_versions` as a list, and it holds more than one entry whenever more than one keg survives in the Cellar — which any skipped or failed `brew cleanup` produces. The wrapper read the first entry, the **oldest**. That version is what the relative verdict (`[SAME]` / `[IMPROVES]`) compares against and what the CVE-aware freshness bypass consults, so a stale keg's CVEs — already fixed in the version actually in use — could stand in for the running one and let a very new release through the `--min-age` hold. Observed on a real run: `simdjson` reported as installed 4.6.4 while brew upgraded from 4.6.7, and a same-day release cleared the 3-day hold on a finding belonging to the older keg. The baseline is now the newest keg brew would actually upgrade from — the last entry, which Homebrew guarantees is the newest because `Formula#outdated_kegs` sorts kegs with `sort_by(&:scheme_and_version)`, skipping `HEAD` kegs for the reason in the next entry. Deferring to brew's own ordering is deliberate — this project's version comparator strips the Homebrew revision (`_N`), so `10.47` and `10.47_1` compare equal and choosing between them here would be arbitrary. The dependency path already took the last entry; the top-level path was the one out of step. Separately, an `installed_versions` present but empty used to raise inside the parser's catch-all and discard **every** outdated package, reporting "Everything up to date." — it no longer can.

- **An excluded package no longer silently suppresses other packages whose names it merely word-matches.** Membership in the exclusion list was tested with `grep -qw "$name"`, which treats the package name as a regular expression and matches on word boundaries — and Homebrew names are full of characters `grep` counts as boundaries. Holding `python@3.12` for freshness therefore dropped `python` as well; `node@22` dropped `node`, `openssl@3` dropped `openssl`, and a held cask dropped a same-named formula. The suppressed package had already been reported `[ok]` by both the age and the CVE check and counted under `Results: N clean` — it then simply vanished, with no line saying why and exit 0. Versioned formulae are ubiquitous and freshness holds are routine, so this was reachable on ordinary runs. Membership is now matched as a literal token, the same correction the dependency path already carried. Nothing unsafe was installed by this — it fails closed — but a package the tool calls clean should be upgraded or explained, not dropped in silence. A formula and a cask sharing a token still exclude each other; that needs type-tagged entries and is tracked separately.

- **A `--HEAD` keg is no longer used as the CVE baseline.** Homebrew ranks a HEAD keg above every numbered version and includes it in the keg list it sorts, so reading the newest keg returned `HEAD-<sha>` whenever a stable and a HEAD install shared a rack. That string cannot be parsed as a version, and an unparseable baseline is reported as carrying every advisory — which fired the freshness bypass and reduced every real regression to `[SAME]`. The newest non-HEAD keg is used instead; a rack holding only HEAD kegs has no usable baseline, so the hold and the block both stand. A blank installed version (which a cask can produce) no longer shifts the parsed columns either — that left the available version empty and the scanner running against no version at all.

- **An unknown installed version is treated as "no baseline", not as "vulnerable to everything".** When an outdated entry carries no usable installed version, the parser emits `?`. That string was passed to the scanner as if it were a version, and `version_in_range()` answers "affected" for anything it cannot parse — correct when scanning a *candidate*, inverted when it is the *baseline*. The baseline came back carrying every CVE in the database, which both fired the freshness bypass (`installed ? has CVEs`) and made any genuine regression look like `[SAME]`, since the candidate's findings are necessarily a subset. Both comparisons are now skipped when the installed version is unknown, so the freshness hold stands and the block stands. A JSON `null` installed version is treated the same way.

- **A package that clears the gate and then fails to install is now named, and the run exits non-zero.** `brew upgrade` declines individual packages and still reports batch success, and the wrapper discarded the result, so nothing verified that the verdict had been carried out. The common trigger is the tool's own pin bracket: packages that could not be vetted are pinned for the duration of the upgrade, and a *cleared* package that needs the latest of one of them is refused by brew with `Error: You must brew unpin … as installing <pkg> requires the latest version of pinned dependencies`. On a real run, `ffmpeg` and `gnupg` passed every check, were announced under "Clean formulae to upgrade", had their bottles downloaded — and were never installed. The run still printed `Done.` and exited 0, so the only signal was two error lines in the middle of brew's output. Every announced package is now re-checked against `brew outdated` after the pins are released; any that did not upgrade are listed with the likely reason and the run exits 1. Checking the end state rather than parsing brew's error text covers the other reasons too (no bottle, build failure, network). The check reads `brew outdated --json=v2`, the same field `CLEAN_PKGS` was built from — `--quiet` prints the alias recorded in the install receipt instead, so a formula installed as `brew install postgresql` (alias for `postgresql@18`) never matched and a refused upgrade read as landed. Three things the check deliberately does not do: it does not report a formula **you** pinned (brew lists pinned formulae as outdated and refuses them by definition, so reporting one would fire on every run forever); it does not pool the formula and cask namespaces, so a still-outdated cask cannot condemn a same-named formula that upgraded; and if the re-check's own `brew outdated` fails it says the upgrade could not be verified rather than reading an empty answer as success — scoped to the namespace actually in play, so a cask-side failure cannot fail an otherwise clean formula-only run — also a non-zero exit, since "could not check" is not "it is fine". The pin itself is unchanged — it is what stops brew resolving an unvetted dependency in underneath another package; what was missing was the report. Nothing unsafe was ever installed by this path; the upgrade simply did not happen and said otherwise.

## [0.4.0] — 2026-09-04

### Changed

- **Advisory prose is no longer treated as version evidence.** When an NVD record carries no CPE data, the affected range used to be read from its description. That reading was wrong in both directions and could not be made right: across six review rounds it dropped live CVEs (a comma list of fix versions cleared a version fixed only in a later entry; a bound truncated at a non-numeric segment cleared a whole release line; quantities in prose — "writes up to 4.0 kilobytes" — parsed as bounds; "remains unpatched in 3.2" matched on the "patched in" substring) and it blocked patched releases (requiring a version to clear bounds belonging to other release lines reported the patched Django 4.2.15 as vulnerable). Every guard against one shape opened another. The parser is therefore **removed**, not repaired: a record with no CPE data is now UNRESOLVED — reported, never cleared on a sentence. **Expect more findings reported** on packages whose CVEs NVD has not analysed. A false positive costs an argument; a false negative ships a vulnerability.

- **A finding blocks only when some other version does not carry it.** Prose was the only thing that could clear a Homebrew finding, since OSV and the GitHub Advisory Database have no Homebrew ecosystem — so removing it alone would have reinstated the bug below. Instead, a finding with no version scope on the **newest available release** is reported but does not block: no version exists without it, and refusing the only version on offer denies the software without reducing exposure. Structured evidence (a CPE or OSV/GHSA range) always blocks. Fails closed whenever the latest version cannot be determined, no version was supplied, or the checked version is not the latest. Such findings appear in the verdict JSON as `unactionable` and are printed by both `brew safe-install` and `brew safe-upgrade` as `note:` lines.

- **`docs/PRD.md` gains a "Verdict semantics" section** — the asymmetry between the two error directions, which evidence is admissible, the AFFECTED/CLEAN/UNRESOLVED states, when blocking is actionable, block-on-what-an-upgrade-adds, and fail-closed-on-error-rather-than-on-ignorance. It was never written down, which is why six review rounds re-litigated it and twice reached opposing conclusions.

### Fixed

- **`brew safe-install` renders the scanner's notes.** `brew safe-upgrade` has printed them since v0.3.4; the install side never grew the same call, so a finding the scanner declined to block on printed nothing at all under a bare `[ok]`.

- **The dependency-relative verdicts `[SAME-DEP]` / `[IMPROVES-DEP]` were unreachable in production.** Both wrappers captured the dependency scan with `2>&1`, and the scanner always writes a header to stderr, so the JSON never parsed, the finding-id list was always empty and the guard always failed — every flagged dependency blocked its dependent permanently, the exact deadlock v0.3.4 claimed to fix. The tests passed only because their stub writes nothing to stderr. Same capture bug erased the reason from every `[skip] — check failed` line in `brew safe-install`.

- **A Homebrew cask declaring `version :latest` no longer hides every CVE.** `brew info` reports the literal string `latest`, and the version parser mapped it to 0, so the CPE range check read `0 < versionStartIncluding` as "outside the affected range" and discarded every CPE-ranged finding for that package — with no OSV or GHSA second opinion for Homebrew to recover them. A resolved version that does not start with a digit now degrades to "unknown", the same path as any other resolution failure. A package name beginning with `-` is also no longer passed to `brew info` as a flag, which previously returned an unrelated version from a full listing.
